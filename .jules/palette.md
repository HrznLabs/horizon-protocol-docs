...
## 2025-05-18 - Tactile Click Feedback for Active States
**Learning:** Translating an element upwards (e.g., `translateY(-2px)`) on its `:active` state opposes the physical direction of a button press, creating a jarring and unnatural interaction, even if it scales down simultaneously.
**Action:** Always ensure that `:active` states for interactive elements (like buttons and cards) mimic a physical press by translating downwards (e.g., `translateY(1px)`), in addition to any slight scale reduction, to provide intuitive and expected tactile feedback.
