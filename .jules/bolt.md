...
## 2025-03-20 - React.memo on Static Components Anti-pattern
**Learning:** Applying `React.memo()` to static components whose parent components have no state and never re-render (such as homepage sections or static icons) is a performance anti-pattern. It adds unnecessary initialization overhead and memory allocation without any actual render-saving benefits.
**Action:** Only use `React.memo()` on components that frequently re-render with identical props, typically when their parent components update state often. Avoid using it for purely static layouts or stateless icons.
