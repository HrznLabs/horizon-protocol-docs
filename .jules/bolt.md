...
## 2024-07-30 - Client-Side Routing in Docusaurus\n**Learning:** Using raw HTML `<a>` tags in Docusaurus markdown files triggers full page reloads, negating the benefits of the single-page application architecture. \n**Action:** Always use the `@docusaurus/Link` component (or standard markdown links) instead of raw `<a>` tags for internal navigation to preserve client-side routing.
