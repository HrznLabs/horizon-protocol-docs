...
## 2025-03-22 - Unnecessary React.memo on Static Components
**Learning:** Applying `React.memo()` to static components whose parent components have no state and never re-render (such as homepage sections or static icons) is a performance anti-pattern. It adds unnecessary initialization overhead and memory allocation without any render-saving benefits.
**Action:** Remove `React.memo()` from static UI components where the parent component does not re-render. Only use `React.memo()` when there are expensive computations or state changes that affect a subset of components while keeping others intact.
