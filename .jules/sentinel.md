...
## 2025-03-29 - ReDoS vulnerability in picomatch via micromatch/tinyglobby
**Vulnerability:** Found a High severity Regular Expression Denial of Service (ReDoS) vulnerability in `picomatch` < 2.3.2 and < 4.0.4. This package was brought in as a transitive dependency via `micromatch`, `anymatch`, `readdirp`, `jest-util`, and `tinyglobby` under `@docusaurus/core` and `@typescript-eslint`.
**Learning:** Using untrusted glob patterns can result in a denial of service if compiled into vulnerable regular expressions by `picomatch`. The project's deep dependency tree had multiple versions of `picomatch` (e.g., `2.3.1` and `4.0.3`) which required careful, multi-path resolution in `package.json` to avoid downgrading modern dependencies like `tinyglobby`.
**Prevention:** Apply targeted dependency paths in `package.json` resolutions (e.g., `"**/tinyglobby/picomatch"`) to safely upgrade multiple instances of a vulnerable package without disrupting the overall dependency tree. Always check `yarn audit` for deeply nested transitive dependencies that rely on glob matching.
